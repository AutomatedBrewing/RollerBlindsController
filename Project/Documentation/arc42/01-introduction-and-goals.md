...
> Rendered from `Project/Model/Analysis/problem`, `Project/Model/Analysis/stakeholders`,
> `Project/Model/Analysis/requirements`. Source of truth: `.sysml` files under `Project/Model/`.
...
**Top-level functional requirements** (`Project/Model/Analysis/requirements/Requirements.sysml`,
...
| REQ-F-09 | pressDurationModeDiscrimination — short press = automatic, long press = manual | mandatory | User |
| REQ-F-10 | timingDriftMargin — configured travel time includes a safety margin | mandatory | User |
...
|---|---|---|---|
| 1 | Safety | REQ-S-01 (SELV ≤50V, galvanic isolation) | Mains-switching device; UP+DOWN conflict must never energize the motor |
...
Source: `Project/Model/Analysis/stakeholders/Stakeholders.sysml`.
...
| User | — (`part def User`) | Convenient shutter control (`ShutterControllerExistence`); compact form factor fitting a ⌀60 flush-mount box (`MinimalSize`); high availability, reliability, and safety (`Availability`, `Reliability`, `SafetyConcern`) |
