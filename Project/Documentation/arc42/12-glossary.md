...
| `TravelTime` | The persisted per-installation `{openTime, closeTime}` pair (REQ-F-07), retrieved at runtime to supply the movement-supervision timeout. |
| `ControlCommandFlow` / `MovementRequestFlow` / `StopRequestFlow` / `MotorStateFlow` / `ShutterStateFlow` / `ConfigModeEventFlow` / `ConflictEventFlow` / `DiagnosticRequestFlow` / `DiagnosticResultFlow` / `FaultIndicationEventFlow` / `TravelTimeFlow` / `TravelTimeFlow` | Typed `flow def` channels, each carrying exactly one corresponding `item def` above between functional/logical elements. |
