...
typedef void (*state_machine_event_logger)(uint32_t state_machine, uint32_t state, void *event);
...
{
    void *Event;          //!< Pending Event for state machine
...
extern "C"
{
    ...
    extern state_machine_result_t dispatch_event(state_machine_t *const pState_Machine[], uint32_t quantity
#if STATE_MACHINE_LOGGER
                                                 ,
...
#if HIERARCHICAL_STATES
    extern state_machine_result_t traverse_state(state_machine_t *const pState_Machine, const state_t *pTarget_State);
#endif // HIERARCHICAL_STATES
