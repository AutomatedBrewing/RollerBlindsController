...
osStatus_t osTimerStart (osTimerId_t timer_id, uint32_t ticks) {
  ...
  if (IS_IRQ()) {
      BaseType_t ret;
      BaseType_t should_perform_context_switch = pdFALSE;
      ret = xTimerChangePeriodFromISR(hTimer, ticks, &should_perform_context_switch);

      if (should_perform_context_switch) {
          portYIELD_FROM_ISR(pdTRUE);
      }
      stat = (ret == pdPASS) ? osOK : osErrorResource;
  }
  else {
...
//  osStatus_t stat;
    ...
    if (IS_IRQ()) {
        BaseType_t should_perform_context_switch = pdFALSE;
        ret = xTimerStopFromISR(hTimer, &should_perform_context_switch);

        if (should_perform_context_switch) {
            portYIELD_FROM_ISR(pdTRUE);
        }
    } else {
        ret = xTimerStop(hTimer, portMAX_DELAY);
    }
